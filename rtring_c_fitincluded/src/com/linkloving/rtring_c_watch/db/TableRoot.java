package com.linkloving.rtring_c_watch.db;
...
import com.eva.android.platf.util.AToolKits;
...
//		db = null;
	...
	public void refreshDataFromServer() throws Exception
	{
		...
		try
		{
//			//** 所取字段的个数及顺序一定要与方法 insertRow(..)中保持一致！
//			//取的是该线路所属的客户！
//			String sql="select " +
//					"khbh,khmc,dh1,case when len(isnull(khdz_new,''))=0 then lxdz else khdz_new end as lxdz" +
//					",lxr1 from kh_v where xlbh = '" 
//				+MyApplication.getInstance(context).getSysUser().getXlbh() + "' and khlx=0";
////			Vector<Vector> data=AToolKits.queryData(sql);
